...
// "Last execution activity" only counts evidence that represents actual
// delivered work (a task update, code commit, or logged hours). Sprint
// Report and Standup entries linked to a Forward gap are typically the
// system or the team *noting the absence* of activity, not activity
// itself, so they're excluded here.
...
// exampleGapId defaults to a real Forward gap from the seed data so this
// card always has something concrete to point at.
export function ForwardGapExplainer({ exampleGapId = "gap-nc-01" }) {
  ...
  return (
    <Card>
      <CardHeader className="flex-row items-center gap-3 space-y-0">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-class-expectation-bg text-class-expectation-fg">
          <TrendingUp className="h-4.5 w-4.5" />
        </div>
        <CardTitle className="text-base">Forward Gap</CardTitle>
      </CardHeader>
      <CardContent>
        {gap && requirement && (
